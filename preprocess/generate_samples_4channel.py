...
start_time = min(event.timestamp, event_1.timestamp)
label_start_time = start_time
#first_event = copy.deepcopy(event)
blank_img = np.zeros((frames_num,) + (4,) + file['events'].size, dtype=np.float32)
...
while True:
    try:
        ...
        # Process events from the first camera
        ...
        # Process events from the second camera
        while event_1.timestamp - start_time < event_frame_time:
            if event_1.polarity:
            ...
            num_events_in_a_sample += 1
            event_1 = next(file['events_1'])
        start_time = min(event.timestamp, event_1.timestamp)  # Update start_time based on the earliest event
        c += 1
        if c == frames_num/2 + 1:
            label_time = start_time #event.timestamp # the timestamp of the first event in 101th frame
            #print('label_time, starting, event_frame_time', label_time, starting, event_frame_time)
            label_index = 1514 + int((label_time - label_start_time - frames_num * event_frame_time * 211)*100/(10**6))
        ...
        if c == frames_num: 
            ...
            if label != None:           
                ...
                event_img_squeeze = np.mean(event_frame_cropped.reshape(50, 4, 128, 2, 128, 2), axis=(3, 5))  
                # 保存事件图像
                if save_count % 50 == 0:
                    way_point_number +=1
                ...
                np.save('/Users/nwang/all_preprocessed_dataset/chop_still_50_2camera_0/chop_still_50_2camera_trip0_{}.npy'.format(save_count), data_dict)
                print("chop_still_50_2camera_trip0_{}.npy is saved!!!".format(save_count), 'number of events is {}!!!!!!!!!'.format(num_events_in_a_sample))
                save_count += 1
