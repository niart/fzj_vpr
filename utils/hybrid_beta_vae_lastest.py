...
"""
The VQAE class is a subclass of the VAE class. It adds a VectorQuantizer object to the model.
The forward method of the VQAE class takes the output of the encoder z and returns the quantized version z_q, the loss term, the perplexity, the indices of the closest embeddings and the one-hot encodings of the closest embeddings.


Please note that the HybridGuidedVAETrainer needs to be modified to fit VQAE
the output and adding loss
...
class VQAE(VAE):
    def __init__(
        ...
        self.quantizer = VectorQuantizer(n_e=n_e, e_dim=e_dim, beta=beta)

    def forward(self, x):
        z = self.encoder(x)
        loss_vq, z_q, perplexity, min_encodings, min_encoding_indices\
            = self.quantizer(z)
        clas = self.cls_sq(z_q)
        return self.decoder(z_q), z, loss_vq, perplexity, min_encodings, min_encoding_indices, clas