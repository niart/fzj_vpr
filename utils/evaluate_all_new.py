...
class HybridGuidedVAETrainer:
    ...
    def create_mydata(
        ...
        **dl_kwargs,
        ...
        )  # Sort filenames based on the integer value within the filename
        file_names_test = os.listdir(root_test)
        file_names_test.sort(
            key=lambda x: int(x.split("_")[-1].split(".")[0])
        )  # Sort filenames based on the integer value within the filename
        ...
        train_dl = torch.utils.data.DataLoader(
            train_d, batch_size=batch_size, shuffle=False, **dl_kwargs
        )
    ...
    def tsne_project_new(
        self, lats, tgts
    ):  ## intended to add an index on every 50 dot, not useful for zero-shot/generalization
        """
