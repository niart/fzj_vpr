...
def generate_process_target(params, aug_epoch=0): 
    ...
    t1 = transforms.ExpFilterEvents(tau=tau2, length = int(6*tau2), tpad=int(6*tau2), device='cuda' ) # the 6 is likely another tunable hyperparameter I can try higher seems to give less quick decay which is good because need to capture more of the movement dynamics  
    t2 = transforms.ExpFilterEvents(tau=tau1, length = int(6*tau1), tpad=int(6*tau1), device='cuda' )
    if aug_epoch<1:
        filter_data = transforms.Compose([t1, t2, transforms.Rescale(50.)]) # 50 is the factor that the values are scaled up i.e. makes the pixels brighter I think
    else:
...
def load_model_from_checkpoint(checkpoint_dir, net, opt, inhib=None, n_checkpoint=-1, device='cuda'):
    '''
