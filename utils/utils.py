...
def generate_process_target(params, aug_epoch=0):
    ...
    t1 = transforms.ExpFilterEvents(
        tau=tau2, length=int(6 * tau2), tpad=int(6 * tau2), device="cuda"
    )  # the 6 is likely another tunable hyperparameter I can try higher seems to give less quick decay which is good because need to capture more of the movement dynamics
    t2 = transforms.ExpFilterEvents(
        tau=tau1, length=int(6 * tau1), tpad=int(6 * tau1), device="cuda"
    )
    if aug_epoch < 1:
        filter_data = transforms.Compose(
            [t1, t2, transforms.Rescale(50.0)]
        )  # 50 is the factor that the values are scaled up i.e. makes the pixels brighter I think
    else:
        filter_data = transforms.Compose(
            [t1, t2, transforms.Rescale(50.0), transforms.Jitter()]
        )

    def process_target(data, aug_epoch=0):
        l = data.shape[1]
        if aug_epoch > 0:
            jitter_data = transforms.Compose(
                [filter_data, transforms.Jitter(xs=2, ys=2, th=5)]
            )
...
def parse_args(default_params_file="parameters/params.yml"):
    parser = argparse.ArgumentParser(
        description="DECOLLE for event-driven object recognition"
    )
    parser.add_argument(
        "--device", type=str, default="cuda", help="Device to use (cpu or cuda)"
    )
    parser.add_argument(
        "--resume_from",
        type=str,
        default=None,
        metavar="path_to_logdir",
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        help="Name of subdirectory to save results in",
    )
    parser.add_argument(
        "--verbose", type=bool, default=False, help="print verbose outputs"
    )
    parser.add_argument("--seed", type=int, default=-1, help="CPU and GPU seed")
    parser.add_argument(
        "--no_train", type=bool, default=False, help="Train model (useful for resume)"
    )
...
def load_model_from_checkpoint(
    checkpoint_dir, net, opt, inhib=None, n_checkpoint=-1, device="cuda"
):
    """
    ...
    if checkpoint_list:
        ...
        if inhib is not None:
            # excite.load_state_dict(checkpoint['excite_state_dict'])
            inhib.load_state_dict(checkpoint["inhib_state_dict"])
...
def get_activities(
    ...
    for data_batch, target_batch in tqdm(iter_gen_train, desc="Epoch {}".format(epoch)):
        ...
        loss_mask = (
            (
                data_batch.reshape(data_batch.shape[0], data_batch.shape[1], -1).mean(2)
                > 0.01
            )
            .unsqueeze(2)
            .float()
        ...
        for k in range(burnin, t_sample):
            ...
            loss_tv += decolle_loss(
                s, r, u, target=target_batch[:, k, :], mask=loss_mask[:, k, :]
            )
            if online_update:
                loss_tv.backward()
                if apply_grad:
...
def train(
    ...
    for data_batch, target_batch in tqdm.tqdm(
        iter_gen_train, desc="Epoch {}".format(epoch)
    ):
        ...
        if batches_per_epoch > 0:
            if batch_iter >= batches_per_epoch:
                break
    ...
    with torch.no_grad():
        ...
        for data_batch, target_batch in tqdm.tqdm(iter_data_labels, desc="Testing"):
            ...
            for k in range(burnin, timesteps):
                s, r, u = net.forward(data_batch[:, k, :, :])
                test_loss_tv = decolle_loss(
                    s, r, u, target=target_batch[:, k], sum_=False
                )
        ...
        if print_error:
            print(
                " ".join(
                    [
                        "Error Rate L{0} {1:1.3}".format(j, 1 - v)
                        for j, v in enumerate(test_acc)
                    ]
                )
            )
    if debug:
...
def save_checkpoint(epoch, checkpoint_dir, net, opt, inhib=None):
        ...
        torch.save(
            {
                ...
                "inhib_state_dict": inhib.state_dict(),
...
def get_output_shape(
    input_shape, kernel_size=[3, 3], stride=[1, 1], padding=[1, 1], dilation=[0, 0]
):
    ...
    height = int(
        (im_height + 2 * padding[0] - dilation[0] * (kernel_size[0] - 1) - 1)
        // stride[0]
        + 1
    )
    width = int(
        (im_width + 2 * padding[1] - dilation[1] * (kernel_size[1] - 1) - 1)
        // stride[1]
        + 1
    )
    return [height, width]
...
class MultiOpt(object):
    def __init__(self, *opts):
        ...
        self.multioptparam = DictMultiOpt(
            [opt.param_groups[-1] for opt in self.optimizers]
        )
