...
def load_model_from_checkpoint(checkpoint_dir, net, opt, excite=None, inhib=None, n_checkpoint=-1, device='cuda'): 
    '''
...
def save_checkpoint(epoch, checkpoint_dir, net, opt, excite=None, inhib=None):
    ...
    if excite is not None:
        torch.save({
            ...
            'excite_state_dict'   : excite.state_dict(),
...
def train_timewrapped(gen_train, loss_fn, net, opt, epoch, online_update=False, reg_loss_fn=None, batches_per_epoch=-1):
    ...
    for data_batch, target_batch in tqdm.tqdm(iter_gen_train, desc='Epoch {}'.format(epoch)):
        ...
        u, ht = net(data_batch, return_sequence = return_sequence)
